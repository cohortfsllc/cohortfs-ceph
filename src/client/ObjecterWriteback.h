...
class ObjecterWriteback : public WritebackHandler {
 public:
  ...
  virtual void read(const object_t& oid,
		    ...
		    Context *onfinish) {
    m_objecter->read_trunc(oid, off, len, snapid, pbl, 0,
  ...
  virtual tid_t write(const object_t& oid,
		      uint64_t off, uint64_t len, const SnapContext& snapc,
		      const bufferlist &bl, utime_t mtime, uint64_t trunc_size,
		      __u32 trunc_seq, Context *oncommit) {
    return m_objecter->write_trunc(oid, off, len, snapc, bl, mtime, 0,
				   trunc_size, trunc_seq, NULL, oncommit);
  }

  virtual tid_t lock(const object_t& oid, int op,
		     int flags, Context *onack, Context *oncommit) {
    return m_objecter->lock(oid, op, flags, onack, oncommit);
  }
