...
rgw_bucket_dir cls_rgw_list_op(rados::Objecter* o, AVolRef& vol,
			       const oid_t& oid, string& start_obj,
			       string& filter_prefix, uint32_t num_entries,
			       bool *is_truncated);

std::tuple<rgw_bucket_dir_header, rgw_bucket_dir_header>
cls_rgw_bucket_check_index_op(rados::Objecter* o, AVolRef& vol,
			      const oid_t& oid);
void cls_rgw_bucket_rebuild_index_op(rados::Objecter* o, AVolRef& vol,
				     const oid_t& oid);

rgw_bucket_dir_header cls_rgw_get_dir_header(rados::Objecter* o, AVolRef& vol,
					     const oid_t& oid);
void cls_rgw_get_dir_header_async(rados::Objecter* o, AVolRef& vol,
...
list<rgw_bi_log_entry> cls_rgw_bi_log_list(rados::Objecter* o, AVolRef& vol,
					   const oid_t& oid, string& marker,
					   uint32_t max,
					   bool *truncated);
void cls_rgw_bi_log_trim(rados::Objecter* o, AVolRef& vol,
			 const oid_t& oid, string& start_marker,
			 string& end_marker);

/* usage logging */
map<rgw_user_bucket, rgw_usage_log_entry>
cls_rgw_usage_log_read(rados::Objecter* o, AVolRef& vol,
...
list<cls_rgw_gc_obj_info> cls_rgw_gc_list(rados::Objecter* o, AVolRef& vol,
					  const oid_t& oid, string& marker,
