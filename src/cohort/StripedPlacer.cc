...
ssize_t StripedPlacer::place(const object_t& object,
			     const OSDMap& map,
...
void StripedPlacer::make_strides(const object_t& oid,
			      ...
			      vector<StrideExtent>& strides) const
{
  ...
  if (curoff % stripe_unit) {
    ...
    strides[stride].bl.substr_of(in, curoff, len);
    curoff += len;
    stride++;
    stride %= stripe_width;
  }
  while (curoff < last_byte + 1) {
    ...
    strides[stride].bl.substr_of(in, curoff, stripe_unit);
    curoff += stripe_unit;
    stride++;
    stride %= stripe_width;
  }
