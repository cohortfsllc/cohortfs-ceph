...
namespace array
{
    ...
    {
        AV_PRECONDITION( util::less_equal( arr.size() + 1, arr.capacity() ) );

        typename Array< _T >::iterator const greaterEqual
            = std::lower_bound( arr.begin(), arr.end(), t, cmp );

        if( greaterEqual != arr.end() )
        {
            bool const isEqual = cmp( t, * greaterEqual ) == false;
        ...
        arr.insert( greaterEqual, std::forward< _T2 >( t ) );
