...
/*
 * "Frags" are a way to describe a subset of a 64-bit number space,
 * using a mask and a value to match against that mask.  Any given frag
 * (subset of the number space) can be partitioned into 2^n sub-frags.
 *
 * Frags are encoded into a 64-bit word:
 *   8 upper bits = "bits"
 *  56 lower bits = "value"
 *
 * We use the _most_ significant bits of the 56 bit value.  This makes
...
typedef uint64_t ceph_frag_t;

#define FRAG_SHIFT 56
#define FRAG_BIT1 (1ull << FRAG_SHIFT)
