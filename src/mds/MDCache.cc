...
void MDCache::create_empty_hierarchy(C_Gather *gather)
{
  ...
  CDirStripe *rootstripe = root->get_placement()->get_or_open_stripe(0);
  ...
  CDirStripe *cephstripe = ceph->get_placement()->get_or_open_stripe(0);
...
void MDCache::create_mydir_hierarchy(C_Gather *gather)
{
  ...
  CDirStripe *mystripe = my->get_placement()->get_or_open_stripe(0);
  ...
  CInode *journal = create_system_inode(MDS_INO_LOG_OFFSET + mds->whoami,
                                        mds->whoami, S_IFREG);
  CDentry *jdn = mydir->add_primary_dentry("journal", journal);
  jdn->_mark_dirty(ls);
...
void MDCache::_create_system_file(CDirFrag *dir, const char *name, CInode *in, Context *fin)
{
  ...
  if (in->is_dir()) {
    ...
    CDirStripe *stripe = in->get_placement()->get_or_open_stripe(0);
    stripe->mark_open();
...
void MDCache::_create_system_file_finish(Mutation *mut, CDentry *dn, Context *fin)
{
  ...
  if (in->is_dir()) {
    CDirStripe *stripe = in->get_placement()->get_stripe(0);
    assert(stripe);
...
void MDCache::open_root()
{
  ...
  if (mds->whoami == mds->mdsmap->get_root()) {
    assert(root->is_auth());
    CDirStripe *stripe = root->get_placement()->get_or_open_stripe(0);
    stripe->set_stripe_auth(mds->whoami);
    CDirFrag *dir = stripe->get_or_open_dirfrag(frag_t());
    if (!dir->is_complete()) {
  ...
  CDirStripe *mystripe = myin->get_placement()->get_or_open_stripe(0);
...
{
  assert(myin);
  CDirPlacement *placement = myin->get_placement();
  CDirStripe *stripe = placement->get_or_open_stripe(0);
  CDirFrag *mydir = stripe->get_or_open_dirfrag(frag_t());
  assert(mydir);
  ...
  if (jdn)
    assert(jdn->get_linkage()->is_primary());
				 ...
				 CInode **pcow_inode, CDentry::linkage_t *dnl)
{
  ...
  if (dnl->is_primary() && dnl->get_inode()->is_multiversion()) {
	...
	follows+1 > dn->first) {
      ...
      dn->first = follows+1;
      CDentry *olddn = dn->dir->add_remote_dentry(dn->get_name(),
                                                  in->ino(), in->d_type(),
						  oldfirst, follows);
...
void MDCache::send_subtree_resolves()
{
  ...
  for (map<CDirStripe*,set<CDirStripe*> >::iterator p = subtrees.begin();
    ...
    if (migrator->is_ambiguous_import(ds)) {
      ...
      for (set<CDirStripe*>::iterator q = subtrees[stripe].begin();
...
void MDCache::handle_resolve(MMDSResolve *m)
{
  ...
  if (mds->is_clientreplay() || mds->is_active() || mds->is_stopping()) {
	...
	for (stripe_bound_map::iterator q = m->subtrees.begin();
	  ...
	  CDirStripe *base = get_dirstripe(q->first);
  ...
  for (map<dirstripe_t, vector<dirstripe_t> >::iterator pi = m->subtrees.begin();
    ...
    CDirStripe *stripe = get_dirstripe(pi->first);
    if (!stripe)
      continue;
...
void MDCache::remove_inode_recursive(CInode *in)
{
  ...
  int count = placement ? placement->get_stripe_count() : 0;
  for (int i = 0; i < count; i++) {
...
void MDCache::rejoin_send_rejoins()
{
  ...
  if (mds->is_rejoin()) {
    for (cap_export_map::iterator p = cap_exports.begin();
         p != cap_exports.end();
	 ++p) {
      map<dirstripe_t, int>::const_iterator t = cap_export_targets.find(p->first);
      assert(t != cap_export_targets.end());
      rejoins[t->second]->cap_exports[p->first] = p->second;
    }
  }
...
void MDCache::handle_cache_rejoin_ack(MMDSCacheRejoin *ack)
{
  ...
  for (map<dirfrag_t, MMDSCacheRejoin::dirfrag_strong>::iterator p = ack->strong_dirfrags.begin();
	...
	if (MDS_INO_MDSDIR(from) == p->first.stripe.ino) {
      ...
      CDirStripe *stripe = placement->get_stripe(p->first.stripe.stripeid);
      if (!stripe) {
        stripe = new CDirStripe(placement, p->first.stripe.stripeid, true);
        placement->add_stripe(stripe);
      }
...
void MDCache::process_exported_caps()
{
  dout(10) << "process_exported_caps" << dendl;

  for (cap_export_map::iterator q = cap_exports.begin();
       q != cap_exports.end();
       ++q) {
    for (client_cap_export_map::iterator r = q->second.begin();
         r != q->second.end();
         ++r) {
      ...
      MClientCaps *m = new MClientCaps(CEPH_CAP_OP_EXPORT, q->first.ino, 0, 0);
...
{
  dirstripe_t ds(in->ino(), CEPH_CAP_OBJECT_INODE);
  client_t client = session->info.get_client();
  ceph_mds_cap_reconnect *rc = get_replay_cap_reconnect(ds, client);
  if (rc) {
...
bool MDCache::trim(int max) 
{
  ...
  if (max == 0 && root) {
    CDirPlacement *placement = root->get_placement();
    list<CDirStripe*> ls;
    placement->get_stripes(ls);
    for (list<CDirStripe*>::iterator p = ls.begin(); p != ls.end(); ++p) {
      CDirStripe *stripe = *p;
      if (stripe->get_num_ref() == 0)
	trim_stripe(stripe, expiremap);
    }
    if (placement->get_num_ref() == 0)
      trim_placement(placement, expiremap);
...
void MDCache::trim_placement(CDirPlacement *placement,
                             map<int, MCacheExpire*>& expiremap)
{
  // STRIPE
  list<CDirStripe*> stripes;
        ...
        if (in->is_dir()) {
          ...
          for (list<CDirStripe*>::iterator sub = stripes.begin(); sub != stripes.end(); ++sub) {
            ...
            if (uncommitted_slave_rename_oldstripe.count(substripe)) {
              ...
              placement->close_stripe(substripe);
          }
        }
...
void MDCache::stripe_remove_replica(CDirStripe *stripe, int from, set<SimpleLock *>& gather_locks)
{
  ...
  while (depth < path.depth()) {
    ...
    if (!curstripe) {
      ...
      if (stripe_auth == mds->get_nodeid()) {
        ...
        curstripe = placement->get_or_open_stripe(stripeid);
      } else {
        discover_dir_stripe(placement, stripeid, _get_waiter(mdr, req, fin),
                            stripe_auth);
        return 1;
      }
    }
 ...
 */
void MDCache::open_remote_dirstripe(CDirPlacement *placement,
                                    stripeid_t stripeid, Context *fin)
{
  dout(10) << "open_remote_dirstripe on " << *placement << dendl;

  assert(!placement->is_auth());
  assert(placement->get_stripe(stripeid) == 0);

  int auth = placement->authority().first;

  if (mds->mdsmap->get_state(auth) >= MDSMap::STATE_REJOIN) {
    discover_dir_stripe(placement, stripeid, fin);
  } else {
    // mds is down or recovering.  forge a replica!
    forge_replica_stripe(placement, stripeid, auth);
    if (fin)
...
void MDCache::open_remote_dentry(CDentry *dn, bool projected, Context *fin)
{
  ...
  open_remote_ino(dnl->get_remote_ino(), fin);
}
				   ...
				   bool discover, bool want_xlocked, int *hint)
{
  ...
  for (unsigned i = 0; i < ancestors.size(); i++) {
    ...
    CDirPlacement *placement = diri->get_placement();
    __u32 dnhash = placement->hash_dentry_name(name);

    // pick stripe
    stripeid_t stripeid = placement->pick_stripe(dnhash);
    CDirStripe *stripe = placement->get_stripe(stripeid);
    if (!stripe) {
      if (placement->get_stripe_auth(stripeid) == mds->get_nodeid()) {
	if (placement->is_frozen()) {
	  dout(10) << " " << *placement << " is frozen, waiting " << dendl;
	  placement->add_waiter(CDirPlacement::WAIT_UNFREEZE,
                                _open_ino_get_waiter(ino, m));
	  return 1;
	}
        stripe = placement->get_or_open_stripe(stripeid);
      } else if (discover) {
        open_remote_dirstripe(placement, stripeid,
                              _open_ino_get_waiter(ino, m));
        return 1;
      }
    }
    ...
    if (stripe) {
      ...
      if (!dir) {
        if (stripe->is_auth()) {
          if (stripe->is_frozen()) {
            ...
            stripe->add_waiter(CDirStripe::WAIT_UNFREEZE, _open_ino_get_waiter(ino, m));
            return 1;
          }
      ...
      if (dir) {
        ...
        if (dir->is_auth()) {
          ...
          open_remote_ino(next_ino, _open_ino_get_waiter(ino, m));
          return 1;
        }
        else if (hint && i == 0)
      ...
      *hint = placement->get_stripe_auth(stripeid);
    break;
  }
  return err;
}
                                              ...
                                              list<Context*>& finished)
{
  ...
  if (placement) {
    placement->decode_replica(p, false);
...
CDirPlacement* MDCache::forge_replica_placement(CInode *in, int from)
{
  assert(mds->mdsmap->get_state(from) < MDSMap::STATE_REJOIN);

  // forge a replica.
  const vector<int> empty_stripe_auth;
  in->set_stripe_auth(empty_stripe_auth);
  CDirPlacement *placement = in->get_placement();
 
  dout(7) << "forge_replica_placement added " << *placement
      << " while mds." << from << " is down" << dendl;

  return placement;
}

CDirStripe* MDCache::add_replica_stripe(bufferlist::iterator& p,
                                        CDirPlacement *placement,
                                        int from, list<Context*>& finished)
{
  ...
  if (stripe) {
    ...
    assert(ds.stripeid < placement->get_stripe_count());
    stripe = placement->add_stripe(new CDirStripe(placement, ds.stripeid, from));
    ...
    placement->take_stripe_waiting(ds.stripeid, finished);
  }
  return stripe;
}

CDirStripe *MDCache::forge_replica_stripe(CDirPlacement *placement,
                                          stripeid_t stripeid, int from)
{
  assert(mds->mdsmap->get_state(from) < MDSMap::STATE_REJOIN);
 
  // forge a replica.
  CDirStripe *stripe = placement->add_stripe(
      new CDirStripe(placement, stripeid, from));
...
void MDCache::handle_dir_update(MDirUpdate *m)
{
  ...
  if (!stripe) {
    ...
    if (m->should_discover()) {
      ...
      CDirPlacement *placement = get_dir_placement(m->get_path().get_ino());
      assert(placement);

      open_remote_dirstripe(placement, ds.stripeid,
...
void MDCache::dispatch_fragment_dir(MDRequest *mdr)
{
  ...
  CDirStripe *stripe = info.dirs.front()->get_stripe();
...
void MDCache::_fragment_stored(MDRequest *mdr)
{
  ...
  assert(it != fragment_requests.end());
  fragment_info_t &info = it->second;
  CDirStripe *stripe = info.resultfrags.front()->get_stripe();
...
void MDCache::show_cache()
{
  ...
  for (inode_map::iterator it = inodes.begin(); it != inodes.end(); ++it) {
    ...
    CDirPlacement *placement = it->second->get_placement();
    dout(7) << "  " << *placement << dendl;
    ...
    for (list<CDirStripe*>::iterator s = stripes.begin(); s != stripes.end(); ++s) {
      ...
      for (list<CDirFrag*>::iterator p = dfs.begin(); p != dfs.end(); ++p) {
        ...
        for (CDirFrag::map_t::iterator q = dir->items.begin();
          ...
          ww << "        " << *dn << std::endl;
          string w = ww.str();
          r = safe_write(fd, w.c_str(), w.length());
          if (r < 0)
            goto out;
        }
      }
