				     ...
				     cohort::SimpleMultiCallback<int>& multi)
{
  ...
  rootdir->commit(0, multi.add_ctx());

  root->store(multi.add_ctx());
}
				     ...
				     cohort::SimpleMultiCallback<int>& multi)
{
  ...
  for (int i = 0; i < NUM_STRAY; ++i) {
    ...
    straydir->commit(0, multi.add_ctx());
  }
  ...
  mydir->commit(0, multi.add_ctx());

  myin->store(multi.add_ctx());
}
...
void MDCache::journal_dirty_inode(MutationImpl *mut, EMetaBlob *metablob,
				  CInode *in)
{
  if (in->is_base()) {
				       ...
				       int flags, int linkunlink)
{
  ...
  while (parent) {
    ...
    MDRequestRef mdmut =
      std::static_pointer_cast<MDRequestImpl,MutationImpl>(mut);
    if (!stop &&
...
void MDCache::send_slave_resolves()
{
  ...
  if (mds->is_resolve()) {
    ...
    for (std::unordered_map<metareqid_t, MDRequestRef>::iterator p = active_requests.begin();
...
void MDCache::trim_unlinked_inodes()
{
  ...
  list<CInode*> q;
  for (std::unordered_map<vinodeno_t,CInode*>::iterator p = inode_map.begin();
       p != inode_map.end();
       ++p) {
    CInode *in = p->second;
...
void MDCache::rejoin_send_rejoins()
{
  ...
  if (!mds->is_rejoin()) {
	...
	if (!(*q)->is_auth()) {
	  ...
	  (*q)->set_object_info(i);
	  if (i.ino)
	    rejoin->add_inode_authpin(vinodeno_t(i.ino), mdr->reqid, mdr->attempt);
	  else
	    rejoin->add_dentry_authpin(i.dirfrag, i.dname, mdr->reqid, mdr->attempt);

	  if (mdr->has_more() && mdr->more()->is_remote_frozen_authpin &&
	      mdr->more()->rename_inode == (*q))
	    rejoin->add_inode_frozen_authpin(vinodeno_t(i.ino),
	...
	if (!(*q)->get_parent()->is_auth()) {
	  ...
	  if (i.ino)
	    rejoin->add_inode_xlock(vinodeno_t(i.ino), (*q)->get_type(),
				    mdr->reqid, mdr->attempt);
	  else
	    rejoin->add_dentry_xlock(i.dirfrag, i.dname,
				     mdr->reqid, mdr->attempt);
	}
      }
      // remote wrlocks
	...
	assert(i.ino);
	rejoin->add_inode_wrlock(vinodeno_t(i.ino), q->first->get_type(),
				 mdr->reqid, mdr->attempt);
      }
    }
  }
  ...
  if (mds->is_rejoin()) {
    ...
    for (CDir::map_t::iterator p = dir->items.begin();
	 ...
	 ++p) {
      ...
      rejoin->add_weak_primary_dentry(dir->ino(), dn->name.c_str(), in->ino());
...
void MDCache::handle_cache_rejoin_weak(MMDSCacheRejoin *weak)
{
  ...
  for (map<inodeno_t,map<string, MMDSCacheRejoin::dn_weak> >::iterator p = weak->weak.begin();
       p != weak->weak.end();
       ++p) {
    CInode *diri = get_inode(p->first);
    if (!diri)
    ...
    for (map<string, MMDSCacheRejoin::dn_weak>::iterator q = p->second.begin();
	 ...
	 ++q) {
      ...
      frag_t fg = diri->pick_dirfrag(q->first);
	...
	dentry_remove_replica(dn, from, gather_locks);
      ...
      if (ack)
	ack->add_strong_dentry(dir->dirfrag(), dn->name,
			       dnl->get_inode()->ino(), 0,
			       dnonce, dn->lock.get_replica_state(), 0);
  ...
  for (std::unordered_map<vinodeno_t,CInode*>::iterator p = inode_map.begin();
	...
	if (dn->is_replica(from) &&
	    (ack == NULL ||
	     ...
	     ack->strong_dentries[dir->dirfrag()].count(dn->name) == 0)) {
	  dentry_remove_replica(dn, from, gather_locks);
...
{
  CInode *in = new CInode(this, true);
  in->inode.ino = ino;
...
void MDCache::handle_cache_rejoin_strong(MMDSCacheRejoin *strong)
{
  ...
  for (map<dirfrag_t, MMDSCacheRejoin::dirfrag_strong>::iterator p = strong->strong_dirfrags.begin();
    ...
    if (!diri)
      diri = rejoin_invent_inode(p->first.ino);
    ...
    map<string,MMDSCacheRejoin::dn_strong>& dmap
      = strong->strong_dentries[p->first];
    for (map<string,MMDSCacheRejoin::dn_strong>::iterator q = dmap.begin();
	 q != dmap.end();
	 ++q) {
      CDentry *dn;
      if (!refragged)
	dn = dir->lookup(q->first);
      else {
	frag_t fg = diri->pick_dirfrag(q->first);
	dir = diri->get_dirfrag(fg);
	assert(dir);
	...
	if (q->second.is_remote()) {
	  dn = dir->add_remote_dentry(q->first, q->second.remote_ino,
				      q->second.remote_d_type);
	...
	if (q->second.is_primary()) {
	  if (vinodeno_t(q->second.ino) != dnl->get_inode()->vino()) {
...
void MDCache::handle_cache_rejoin_ack(MMDSCacheRejoin *ack)
{
  ...
  for (map<dirfrag_t, MMDSCacheRejoin::dirfrag_strong>::iterator p = ack->strong_dirfrags.begin();
    ...
    // dentries
    map<string, MMDSCacheRejoin::dn_strong>& dmap = ack->strong_dentries[p->first];
    for (map<string, MMDSCacheRejoin::dn_strong>::iterator q = dmap.begin();
	 q != dmap.end();
	 ++q) {
      CDentry *dn = dir->lookup(q->first);
	...
	CInode *in = dnl->get_inode();
	if (!q->second.is_primary() ||
	    vinodeno_t(q->second.ino) != in->vino()) {
  ...
  for (map<inodeno_t,map<client_t,Capability::Import> >::iterator p = peer_imported.begin();
    ...
    for (map<client_t,Capability::Import>::iterator q = p->second.begin();
	 ...
	 ++q) {
      ...
      MClientCaps *m = new MClientCaps(CEPH_CAP_OP_EXPORT, p->first,
				       cap_exports[p->first][q->first].cap_id,
				       0);
...
void MDCache::choose_lock_states_and_reconnect_caps()
{
  ...
  for (std::unordered_map<vinodeno_t,CInode*>::iterator i = inode_map.begin();
       i != inode_map.end();
       ++i) {
    CInode *in = i->second;
...
void MDCache::do_file_recover()
{
  ...
  while (file_recovering.size() < 5 &&
    ...
    if (pi->client_ranges.size() && pi->get_max_size()) {
      ...
      oid_t oid = CInode::get_object_name(in->inode.ino, frag_t(), "");
      // 0 or CEPH_OSD_FLAG_RWORDERED?
      mds->objecter->stat(oid, in->volume, 0,
			  [this, in](int r, uint64_t s, ceph::real_time t) {
			    _recovered(in, r, s, t);
			  });
    } else {
...
void MDCache::purge_prealloc_ino(inodeno_t ino, OSDC::op_callback&& fin)
{
...
// XXX error recovery?
  ...
  mds->objecter->remove(oid, volume, ceph::real_clock::now(), 0, 0,
			std::move(fin));
}
...
struct CB_MDC_TruncateFinish {
  MDCache *mdc;
  CInode *in;
  LogSegment *ls;
  CB_MDC_TruncateFinish(MDCache *c, CInode *i, LogSegment *l) :
    mdc(c), in(i), ls(l) {}
  void operator()(int r) {
    assert(r == 0 || r == -ENOENT);
...
{
  inode_t *pi = &in->inode;
  oid_t oid = CInode::get_object_name(pi->ino, frag_t(), "");
  ...
  mds->objecter->trunc(oid, in->volume, ceph::real_clock::now(), 0,
		       pi->truncate_size, pi->truncate_seq,
		       0, CB_MDC_TruncateFinish(this, in, ls));
}
...
bool MDCache::trim_dentry(CDentry *dn, map<int, MCacheExpire*>& expiremap)
{
  ...
  if (!dn->is_auth()) {
	...
	expiremap[a] = new MCacheExpire(mds->get_nodeid());
      expiremap[a]->add_dentry(con->dirfrag(), dir->dirfrag(), dn->name,
			       dn->get_replica_nonce());
    }
  }
...
void MDCache::trim_non_auth()
{
  ...
  if (lru.lru_get_size() == 0) {
    // root, stray, etc.?
    std::unordered_map<vinodeno_t,CInode*>::iterator p = inode_map.begin();
    while (p != inode_map.end()) {
      std::unordered_map<vinodeno_t,CInode*>::iterator next = p;
      ++next;
      CInode *in = p->second;
      if (!in->is_auth()) {
	list<CDir*> ls;
...
void MDCache::handle_cache_expire(MCacheExpire *m)
{
  ...
  for (map<dirfrag_t,MCacheExpire::realm>::iterator p = m->realms.begin();
    ...
    // DENTRIES
    for (map<dirfrag_t, map<string,uint32_t> >::iterator pd
	   = p->second.dentries.begin();
	...
	assert(dir->is_auth());
      ...
      for (map<string,uint32_t>::iterator p = pd->second.begin();
	   p != pd->second.end();
	   ++p) {
...
CInode *MDCache::cache_traverse(const filepath& fp)
{
  ...
  for (unsigned i = 0; i < fp.depth(); i++) {
    ...
    if (!curdir)
      return NULL;
    CDentry *dn = curdir->lookup(dname);
    if (!dn)
      return NULL;
...
class CB_MDC_OpenInoBacktraceFetched {
  MDCache *cache;
  inodeno_t ino;
  public:
  bufferlist bl;
  CB_MDC_OpenInoBacktraceFetched(MDCache *c, inodeno_t i) :
    cache(c), ino(i) {}
  void operator()(int r) {
...
// XXX error recovery?
	...
	return;
      ...
      OSDC::op_callback fin(CB_MDC_OpenInoBacktraceFetched(this, ino));
      fetch_backtrace(ino, info.volume,
		      fin.target<CB_MDC_OpenInoBacktraceFetched>()->bl,
		      std::move(fin));
      return;
    }
  } else if (err == -ENOENT) {
				   ...
				   bool discover, bool want_xlocked, int *hint)
{
  ...
  for (unsigned i = 0; i < ancestors.size(); i++) {
	...
	if (!dnl) {
	  ...
	  discover_path(dir, path, _open_ino_get_waiter(ino, m),
			(i == 0 && want_xlocked));
	  return 1;
	}
	...
	if (!info.ancestors.empty()) {
	  ...
	  if (diri) {
	    ...
	    if (dir && !dir->is_auth()) {
	      ...
	      discover_path(dir, path, NULL, true);
	    }
	  }
	}
...
{
  std::unordered_map<metareqid_t, MDRequestRef>::iterator p = active_requests.find(rid);
...
class MDC_PurgeStrayPurged : public cohort::SimpleMultiCallback<int> {
...
public:
  ...
  CDentry *dn;
private:
  MDC_PurgeStrayPurged(MDCache *c, CDentry *d)
    : cache(c), dn(d) { }
public:
  void work(int _r) {
    assert(_r == 0 || _r == -ENOENT);
    if (_r == -ENOENT)
      r = _r;
  }
  void finish() {
...
// XXX error recovery?
    ...
    for (list<frag_t>::iterator p = ls.begin();
	 ...
	 ++p) {
      ...
      mds->objecter->remove(oid, volume, ceph::real_clock::now(),
			    0, NULL, purged.add());
    }
    purged.activate();
    return;
  ...
  if (in->is_file()) {
    ...
    if (to && period) {
      ...
      dout(10) << "purge_stray 0~" << to << " objects 0~" << num
	       << " on " << *in << dendl;
      mds->objecter->zero(oid, in->volume, 0, 0,
  ...
  inode_t *pi = in->get_projected_inode();
  oid_t poid = CInode::get_object_name(pi->ino, frag_t(), "");
...
{
  MDiscover *dis = new MDiscover(d.ino, d.frag,
...
struct C_MDC_RetryDiscoverPath : public Context {
  ...
  filepath path;
  int from;
  C_MDC_RetryDiscoverPath(MDCache *c, CInode *b, filepath &p, int f) :
    mdc(c), base(b), path(p), from(f)  {}
  void finish(int r) {
    mdc->discover_path(base, path, 0, from);
  }
};
			    ...
			    int from)
{
  ...
  dout(7) << "discover_path " << base->ino() << " " << want_path
	  << " from mds." << from
  ...
  if (base->is_ambiguous_auth()) {
    ...
    if (!onfinish)
      onfinish = new C_MDC_RetryDiscoverPath(this, base, want_path, from);
...
struct C_MDC_RetryDiscoverPath2 : public Context {
  ...
  CDir *base;
  filepath path;
  C_MDC_RetryDiscoverPath2(MDCache *c, CDir *b, filepath &p) :
    mdc(c), base(b), path(p) {}
  void finish(int r) {
    mdc->discover_path(base, path, 0);
  }
};
			    ...
			    bool want_xlocked)
{
  ...
  dout(7) << "discover_path " << base->dirfrag() << " " << want_path
	  << " from mds." << from
	  << (want_xlocked ? " want_xlocked":"")
	  << dendl;
  ...
  if (base->is_ambiguous_auth()) {
    ...
    if (!onfinish)
      onfinish = new C_MDC_RetryDiscoverPath2(this, base, want_path);
  ...
  if ((want_xlocked && want_path.depth() == 1) ||
      !base->is_waiting_for_dentry(want_path[0].c_str()) || !onfinish) {
    discover_info_t& d = _create_discover(from);
    d.ino = base->ino();
  ...
  */
  ...
  if (m->is_flag_error_dir() && !cur->is_dir()) {
    ...
    if (m->get_error_dentry().length()) {
      // wanted a dentry
      if (dir && dir->is_waiting_for_dentry(m->get_error_dentry())) {
	if (dir->is_auth() || dir->lookup(m->get_error_dentry())) {
	  dir->take_dentry_waiting(m->get_error_dentry(), finished);
	} else {
	  filepath relpath(m->get_error_dentry(), 0);
	  discover_path(dir, relpath, 0, m->get_wanted_xlocked());
	}
      } else
...
class MDC_FragmentFinish : public cohort::SimpleMultiCallback<int> {
  friend class cohort::MultiCallback;
  MDCache *mdcache;
  dirfrag_t basedirfrag;
  list<CDir*> resultfrags;
  MDC_FragmentFinish(MDCache *m, dirfrag_t f, list<CDir*>& l) :
...
void MDCache::_fragment_committed(dirfrag_t basedirfrag, list<CDir*>& resultfrags)
{
  ...
  for (list<frag_t>::iterator p = uf.old_frags.begin();
       p != uf.old_frags.end();
       ++p) {
    oid_t oid = CInode::get_object_name(basedirfrag.ino, *p, "");
    ...
    mds->objecter->mutate(oid, volume, op, ceph::real_clock::now(),
			  0, NULL, finished.add());
  }
...
void MDCache::show_cache()
{
  ...
  for (std::unordered_map<vinodeno_t,CInode*>::iterator it = inode_map.begin();
       it != inode_map.end();
       ++it) {
    // unlinked?
    if (!it->second->parent)
