...
protected:
  map<dirstripe_t,int>              import_state;
  map<dirstripe_t,int>              import_peer;
  map<dirstripe_t,Message*>         import_pending_msg;
...
public:
 ...
 protected:
  ...
  void export_go(CStripe *stripe);
  void export_go_synced(CStripe *stripe);
  void export_reverse(CStripe *stripe);
  void export_notify_abort(CStripe *stripe, set<CStripe*>& bounds);
...
protected:
  ...
  void import_reverse_prepping(CStripe *stripe);
  void import_remove_pins(CStripe *stripe, set<CStripe*>& bounds);
  void import_reverse_unfreeze(CStripe *stripe);
  void import_reverse_final(CStripe *stripe);
  void import_notify_abort(CStripe *stripe, set<CStripe*>& bounds);
  void import_notify_finish(CStripe *stripe, set<CStripe*>& bounds);
  void import_logged_start(CStripe *stripe, int from,
			   map<client_t,entity_inst_t> &imported_client_map,
			   map<client_t,uint64_t>& sseqmap);
  void handle_export_finish(MExportDirFinish *m);
public:
  void import_finish(CStripe *dir, bool notify);
protected:
