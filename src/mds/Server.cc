    ...
    while (!session->caps.empty()) {
      ...
      if (o) {
        dout(20) << " killing capability " << ccap_string(cap->issued())
            << " on " << *o << dendl;
        o->remove_client_cap(session->info.inst.name.num());
      }
    }
  ...
  for (map<dirstripe_t, cap_reconnect_t>::iterator p = m->caps.begin();
    ...
    if (p->first.stripeid == CEPH_CAP_OBJECT_INODE) {
      ...
      if (stripe && stripe->is_auth()) {
        ...
        dout(15) << "open cap realm " << inodeno_t(p->second.capinfo.snaprealm)
            << " on " << *stripe << dendl;
        stripe->reconnect_cap(client, p->second.capinfo, session);
        continue;
      }
...
void Server::handle_client_readdir(MDRequest *mdr)
{
  ...
  if (offset_str.empty())
    ...
    it = dir->items.upper_bound(dentry_key_t(mdr->snapid, &offset_str));
  ...
  for (CDirFrag::map_t::iterator it = dir->begin();
       !end && numfiles < max;
       end = (it == dir->end())) {
...
void Server::handle_client_unlink(MDRequest *mdr)
{
  ...
  if (in->is_dir()) {
	...
	dout(10) << " already waiting on witness mds." << *p << dendl;
      else if (!_rmdir_prepare_witness(mdr, dn, dnl, in, *p))
        return;
    }
    if (!mdr->more()->waiting_on_slave.empty())
...
bool Server::_rmdir_prepare_witness(MDRequest *mdr, CDentry *dn,
...
void Server::handle_slave_rename_prep(MDRequest *mdr)
{
  ...
  if (destin && destin->is_auth()) {
    ...
    le->commit.add_inode(destin, true, inoparent_t(), req->dest.dn);
  }
...
void Server::_commit_slave_rename(MDRequest *mdr, int r)
{
  ...
  if (r == 0) {
	...
	if (mdr->more()->rename_inode->is_auth())
