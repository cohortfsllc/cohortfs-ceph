...
void Server::handle_client_openc(MDRequest *mdr)
{
  ...
  journal_allocated_inos(mdr, &le->metablob);
  le->metablob.add_primary_dentry(inodn, in, true, true);
...
void Server::handle_client_mknod(MDRequest *mdr)
{
  ...
  le->metablob.add_primary_dentry(inodn, newi, true, true);
...
void Server::handle_slave_mkdir(MDRequest *mdr)
{
  ...
  le->commit.add_primary_dentry(dn, in, false);
