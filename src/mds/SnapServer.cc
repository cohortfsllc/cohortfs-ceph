...
void SnapServer::check_osd_map(bool force)
{
  ...
  mds->objecter->with_osdmap([&](const OSDMap& o) {
      for (const auto& kv : need_to_purge) {
	int id = kv.first;
	const pg_pool_t *pi = o.get_pg_pool(id);
	for (const auto& s : kv.second) {
	  if (pi->is_removed_snap(s)) {
	    dout(10) << " osdmap marks " << s << " as removed" << dendl;
	    all_purged[id].push_back(s);
	  } else {
	    all_purge[id].push_back(s);
	  }
	}
      }
    });
