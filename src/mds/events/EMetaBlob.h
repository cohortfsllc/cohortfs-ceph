...
public:
   ...
   *
   * We encode this one a bit weirdly, just because (also, it's
   * marginally faster on multiple encodes, which I think can happen):
   * Encode a bufferlist on struct creation with all data members,
   * without a struct_v.  When encode is called, encode struct_v and
   * then append the bufferlist.  Decode straight into the appropriate
   * variables.
   *
  ...
  struct remotebit {
    ...
    remotebit(const string& d, version_t v, inodeno_t i, unsigned char dt,
	      bool dr) :
      _enc(256) {
      ::encode(d, _enc);
      ::encode(v, _enc);
    ...
    remotebit(): dnv(0), ino(0), d_type('\0'), dirty(false) {}
  ...
  struct nullbit {
    ...
    nullbit(const string& d, version_t v, bool dr) :
      _enc(128) {
      ::encode(d, _enc);
      ::encode(v, _enc);
      ::encode(dr, _enc);
