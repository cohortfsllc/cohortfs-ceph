...
int Pipe::accept()
{
  ...
  while (1) {
    ...
    if (connect.authorizer_len) {
      bp = ceph::buffer::raw::create(connect.authorizer_len);
...
int Pipe::connect()
{
  ...
  while (1) {
    ...
    if (reply.authorizer_len) {
      ...
      bufferptr bp = ceph::buffer::raw::create(reply.authorizer_len);
...
static void alloc_aligned_buffer(bufferlist& data, unsigned len, unsigned off)
{
  ...
  if (off & ~CEPH_PAGE_MASK) {
    ...
    bufferptr bp = ceph::buffer::raw::create(head);
    data.push_back(bp);
    left -= head;
  }
  unsigned middle = left & CEPH_PAGE_MASK;
  if (middle > 0) {
    bufferptr bp = ceph::buffer::raw::create_page_aligned(middle);
    data.push_back(bp);
    left -= middle;
  }
  if (left) {
    bufferptr bp = ceph::buffer::raw::create(left);
    data.push_back(bp);
  }
}
...
int Pipe::read_message(Message **pm, AuthSessionHandler* auth_handler)
{
  ...
  if (front_len) {
    bufferptr bp = ceph::buffer::raw::create(front_len);
  ...
  if (middle_len) {
    bufferptr bp = ceph::buffer::raw::create(middle_len);
  ...
  if (data_len) {
	...
	if (rxbuf.length() == 0 || p->second.second != rxbuf_version) {
	  ...
	  if (rxbuf.length() < data_len)
	    rxbuf.push_back(ceph::buffer::raw::create(data_len - rxbuf.length()));
