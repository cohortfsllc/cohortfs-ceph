...
int FileStore::lfn_find(FSCollection* fc, const hobject_t& oid)
{
  struct stat st[1];
...
int FileStore::lfn_stat(FSCollection* fc, const hobject_t& oid,
	     struct stat *st)
{
  int r = ::fstatat(fc->fd, oid.to_str().c_str(), st, AT_SYMLINK_NOFOLLOW);
  if (r < 0)
			...
			FDRef *outfd)
{
  ...
  int fd;
  if (!replaying) {
    Mutex::Locker l(fc->fdcache_lock);
...
int FileStore::lfn_unlink(FSCollection* fc, const hobject_t& o,
...
int FileStore::open_journal()
{
  if (journalpath.length()) {
    ...
    journal = new FileJournal(
      this, fsid, &finisher, &sync_cond, journalpath.c_str(),
...
int FileStore::dump_journal(ostream& out)
{
  ...
  FileJournal *journal = new FileJournal(
    this, fsid, &finisher, &sync_cond, journalpath.c_str(), m_journal_dio);
  r = journal->dump(out);
  delete journal;
  return r;
}
...
bool FileStore::exists(CollectionHandle ch, const hobject_t& oid)
{
  struct stat st;
  ...
  bool allow_eio)
{
  ...
  if (got < 0) {
    dout(10) << "FileStore::read(" << ch->get_cid() << "/"
	     << fo->get_oid() << ") pread error: " << cpp_strerror(got)
	     << dendl;
  ...
  if (m_filestore_sloppy_crc && (!replaying || backend->can_checkpoint())) {
    ostringstream ss;
    int errors = backend->_crc_verify_read(**fo->fd, offset, got, bl, &ss);
    if (errors > 0) {
      dout(0) << "FileStore::read " << ch->get_cid() << "/" << fo->get_oid()
	      << " " << offset << "~" << got << " ... BAD CRC:\n" << ss.str()
	      << dendl;
  ...
  dout(10) << "FileStore::read " << ch->get_cid() << "/" << fo->get_oid()
	   << " " << offset << "~" << got << "/" << len << dendl;
  if (cct->_conf->filestore_debug_inject_read_err &&
...
int FileStore::_remove(FSCollection* fc, const hobject_t& oid,
		       const SequencerPosition &spos)
{
			...
			map<string,bufferptr>& aset, bool user_only)
{
  ...
  dout(15) << "getattrs " << ch->get_cid() << "/" << oh->get_oid() << dendl;
...
int FileStore::_rmattrs(FSCollection* fc, FSObject* fo,
			const SequencerPosition &spos)
{
  dout(15) << "rmattrs " << fc->get_cid() << "/" << fo->get_oid() << dendl;
...
int FileStore::collection_list(CollectionHandle ch, vector<hobject_t>& ls)
{
...
int FileStore::_destroy_collection(FSCollection* fc)
{
  ...
  do {
    ulf = false;
    while ((r = ::readdir_r(d, (struct dirent *)&fn, &dt)) == 0 && dt) {
