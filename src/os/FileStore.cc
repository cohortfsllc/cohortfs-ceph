		     ...
		     const bufferlist& bl, bool replica)
{
  ...
  struct iovec *iov;
  int ilen, iov_ix, n_iov = min(256UL, bl.buffers().size());
  iov = static_cast<struct iovec*>(::alloca(n_iov));

  for (ilen = 0 /*, pb = bl.begin() */; pb != bl.buffers().end(); ++pb) {
    for (iov_ix = 0; (iov_ix < n_iov) && (pb != bl.buffers().end()); ++iov_ix, ++pb) {
...
void FileStore::sync_entry()
{
  ...
  while (!stop) {
    ...
    if (!force_sync) {
      dout(20) << "sync_entry waiting for max_interval " << max_interval
	       << dendl;
