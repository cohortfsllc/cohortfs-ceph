...
FragTreeIndex::FragTreeIndex(uint32_t initial_split)
...
int FragTreeIndex::init(const std::string &path)
{
  ...
  if (initial_split > 0) {
    ...
    RWLock::WLocker lock(index_lock);
    r = write_index(dirfd);
  }
  ::close(dirfd);
  return r;
...
int FragTreeIndex::mount(const std::string &path, bool async_recovery)
{
  ...
  index_lock.get_write();
  int r = read_index(rootfd);
  index_lock.put_write();
  if (r)
    goto out_close;
...
int FragTreeIndex::stat(const std::string &name, uint64_t hash, struct stat *st)
{
  ...
  struct frag_path path, orig;
  {
    ...
    RWLock::RLocker lock(index_lock);
...
void FragTreeIndex::increment_size(frag_t frag)
{
  ...
  // split if necessary
  if (count >= g_conf->fragtreeindex_split_threshold)
    split(frag, g_conf->fragtreeindex_split_bits);
}
...
void FragTreeIndex::decrement_size(frag_t frag, frag_t parent)
{
  ...
  if (sum < g_conf->fragtreeindex_merge_threshold)
    merge(parent);
}
...
int split_mkdirs(int rootfd, const fragtree_t &tree, frag_t frag, int bits)
{
...
int FragTreeIndex::split(frag_t frag, int bits, bool async)
{
  ...
  int r = split_mkdirs(rootfd, tree, frag, bits);
  if (r) {
...
int FragTreeIndex::merge(frag_t frag, bool async)
{
  ...
  RWLock::WLocker ilock(index_lock);
