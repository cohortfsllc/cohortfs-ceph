...
int HashIndex::_lookup(const hobject_t &oid,
		       vector<string> *path,
		       string *mangled_name,
		       int *exists_out) {
...
void HashIndex::get_path_components(const hobject_t &oid,
				    vector<string> *path) {
  char buf[MAX_HASH_LEVEL + 1];
  snprintf(buf, sizeof(buf), "%.*X", MAX_HASH_LEVEL, (uint32_t)oid.get_filestore_key());
...
string HashIndex::get_path_str(const hobject_t &oid) {
  assert(!oid.is_max());
  return get_hash_str(oid.hash);
}
...
int HashIndex::list_by_hash(const vector<string> &path,
  ...
  for (set<string>::iterator i = hash_prefixes.begin();
	...
	if (next)
	  *next = hobject_t(hobject_t("", "", hash_prefix_to_hash(*i), -1, ""));
	return 0;
      }
  ...
  if (next)
    *next = hobject_t(hobject_t::get_max());
  return 0;
}
