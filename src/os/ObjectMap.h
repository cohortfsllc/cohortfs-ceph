...
public:
  ...
  virtual int set_keys(
    const oid &obj, ///< [in] object containing map
  ...
  /// Clear all map keys and values from obj
  virtual int clear(
    const oid &obj, ///< [in] object containing map
    const SequencerPosition *spos=0     ///< [in] sequencer position
    ) = 0;

  /// Clear all map keys and values in to_clear from obj
  virtual int rm_keys(
    const oid &obj, ///< [in] object containing map
  ...
  /// Clone keys efficiently from obj map to target map
  virtual int clone(
    const oid &obj, ///< [in] object containing map
    const oid &target, ///< [in] target of clone
    const SequencerPosition *spos=0 ///< [in] sequencer position
    ) { return 0; }

  /// Ensure all previous writes are durable
  virtual int sync(
    const oid *obj=0, ///< [in] object
  ...
  virtual ObjectMapIterator get_iterator(const oid &obj) {
