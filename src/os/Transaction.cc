...
int Transaction::push_col(Collection* ch)
{
  auto cid = ch->get_cid();
...
int Transaction::push_obj(Object* oh)
{
  auto oid = oh->get_oid();
...
void Transaction::close_collections()
{
  if (os) {
...
void encode(const obj_slot_t &o_slot, bufferlist &bl)
{
  // XXX conditionally inspect handle and encode it's cid, if present
  using std::get;
  Object* o = get<0>(o_slot);
  if (!!o)
    encode(o->get_oid(), bl);
  else
    encode(get<1>(o_slot), bl);
  const auto flags = get<2>(o_slot) & ~Transaction::FLAG_REF;
  encode(flags, bl);
}
...
void Transaction::Op::decode(bufferlist::iterator& p)
{
  coll_t cid;
  ::decode(op, p);
  switch (op) {
