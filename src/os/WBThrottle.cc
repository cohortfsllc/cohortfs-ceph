...
bool WBThrottle::get_next_should_flush(unique_lock& l,
  boost::tuple<hoid_t, FDRef, PendingWB> *next)
{
  assert(l.owns_lock());
  assert(next);
  while (!stopping &&
  ...
  hoid_t oid(pop_object());

  map<hoid_t, pair<PendingWB, FDRef> >::iterator i =
...
{
  unique_lock l(lock);
  boost::tuple<hoid_t, FDRef, PendingWB> wb;
  while (get_next_should_flush(l, &wb)) {
    ...
    cond.notify_all();
    wb = boost::tuple<hoid_t, FDRef, PendingWB>();
  }
  return 0;
}

void WBThrottle::queue_wb(
  FDRef fd, const hoid_t &hoid, uint64_t offset, uint64_t len,
  bool nocache)
{
  lock_guard l(lock);
...
void WBThrottle::clear_object(const hoid_t &hoid)
{
  unique_lock l(lock);
  while (clearing == hoid)
    cond.wait(l);
