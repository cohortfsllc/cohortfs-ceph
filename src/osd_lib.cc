...
extern "C" {
  ...
  int libosd_main(int argc, const char **argv)
  {
	...
	!g_conf->cluster_addr.is_blank_ip()) {
      derr << TEXT_YELLOW
	   << " ** WARNING: specified cluster addr but not public addr; "
    ...
    Messenger *client_messenger =
      Messenger::create(g_ceph_context, entity_name_t::OSD(whoami), "client",
			getpid());
    Messenger *cluster_messenger =
      Messenger::create(g_ceph_context, entity_name_t::OSD(whoami), "cluster",
			getpid());
    Messenger *messenger_hbclient =
      Messenger::create(g_ceph_context, entity_name_t::OSD(whoami), "hbclient",
			getpid());
    Messenger *messenger_hbserver =
      Messenger::create(g_ceph_context, entity_name_t::OSD(whoami), "hbserver",
			getpid());
    ...
    cluster_messenger->set_default_policy(
      Messenger::Policy::stateless_server(0, 0));

    cluster_messenger->set_policy(
      entity_name_t::TYPE_MON,
    ...
    messenger_hbclient->set_policy(
      entity_name_t::TYPE_OSD,
      Messenger::Policy::lossy_client(0, 0));

    messenger_hbserver->set_policy(
      entity_name_t::TYPE_OSD,
      Messenger::Policy::stateless_server(0, 0));
    ...
    if (err < 0) {
      derr << TEXT_RED << " ** ERROR: osd pre_init failed: "
  ...
  int libosd_signal(int sig, uint32_t flags)
  {
    int rc = 0;
  ...
  int libosd_read(void *stuff, uint32_t flags)
  {

    return (0);
  } /* libosd_read */

  int libosd_write(void *stuff, uint32_t flags)
  {

    return (0);
  } /* libosd_write */

  int libosd_commit(void *stuff)
  {
    return (0);
  } /* libosd_commit */
