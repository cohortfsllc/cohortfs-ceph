 ...
 public:
  ...
  int truncate(inodeno_t ino,
    ...
    if (extents.size() == 1) {
      ...
      objecter->_modify(extents[0].oid, ops, mtime, snapc, flags, onack, oncommit);
    } else {
	...
	ops[0].op.extent.truncate_seq = truncate_seq;
	objecter->_modify(p->oid, ops, mtime, snapc, flags,
  ...
  int zero(inodeno_t ino,
	...
	objecter->remove(extents[0].oid,
			 snapc, mtime, flags, onack, oncommit);
      else
	objecter->zero(extents[0].oid, extents[0].offset, extents[0].length, 
		       snapc, mtime, flags, onack, oncommit);
    } else {
      C_GatherBuilder gack(cct, onack);
      C_GatherBuilder gcom(cct, oncommit);
	...
	if (p->offset == 0 && p->length == layout->fl_object_size)
	  objecter->remove(p->oid,
			   snapc, mtime, flags,
			   onack ? gack.new_sub():0,
			   oncommit ? gcom.new_sub():0);
	else
	  objecter->zero(p->oid, p->offset, p->length, 
			 snapc, mtime, flags,
			 onack ? gack.new_sub():0,
			 oncommit ? gcom.new_sub():0);
      }
