...
#ifndef OSDC_OBJECTER_BASE_H
#define OSDC_OBJECTER_BASE_H
...
namespace rados {
  ...
  using rados_detail::thunk;
  using rados_detail::op_callback;
  using rados_detail::statfs_callback;
  ...
  namespace rados_detail {
    ...
    using cohort::acquire_unique;
    using cohort::FunQueue;
    using cohort::is_iterator;
    using ceph::timespan;
	...
	if (osdmap->get_epoch()) {
	  ...
	  for (epoch_t e = osdmap->get_epoch() + 1;
	    ...
	    static_cast<D*>(this)->_scan_sessions(skipped_map, was_full,
						  need_resend);
	  ...
	  if (m->maps.count(m->get_last())) {
	    ...
	    _scan_sessions(false, false, need_resend);
	    ldout(cct, 3) << "handle_osd_map decoding full epoch "
			  << m->get_last() << dendl;
	    osdmap->decode(m->maps[m->get_last()]);
	    static_cast<D*>(this)->_scan_requests(homeless_subops, shl, false,
						  false, need_resend);

	  } else {
	    ldout(cct, 3)
	      << "handle_osd_map hmm, i want a full map,requesting" << dendl;
	...
	static_cast<D*>(this)->_resend_subop(kv.second);
    ...
    template<typename D>
    void Objecter<D>::wait_for_osd_map() {
      unique_lock wl(rwlock);
      if (osdmap->get_epoch()) {
	wl.unlock();
	return;
    ...
    template<typename D>
    void Objecter<D>::add_osdmap_notifier(thunk&& f) {
    ...
    struct Objecter_GetVersion {
      Objecter<D>& objecter;
      thunk fin;
      Objecter_GetVersion(Objecter<D>& o, thunk&& f)
	: objecter(o), fin(f) {}
    ...
    template<typename D>
    void Objecter<D>::get_latest_version(epoch_t oldest, epoch_t newest,
					 thunk&& fin) {
      unique_lock wl(rwlock);
    ...
    template<typename D>
    void Objecter<D>::_get_latest_version(epoch_t oldest, epoch_t newest,
				     thunk&& fin) {
    ...
    template<typename D>
    void Objecter<D>::maybe_request_map() {
      shared_lock rl(rwlock);
      _maybe_request_map();
    }

    template<typename D>
    void Objecter<D>::_wait_for_new_map(thunk&& c, epoch_t epoch) {
      waiting_for_map[epoch].add(std::move(c));
      static_cast<D*>(this)->_maybe_request_map();
    }

    template<typename D>
    bool Objecter<D>::wait_for_map(epoch_t epoch, thunk&& c) {
      unique_lock wl(rwlock);
    ...
    template<typename D>
    ceph_tid_t Objecter<D>::op_submit(Op* op) {
    ...
    template<typename D>
    void Objecter<D>::op_cancel(ceph_tid_t tid, std::error_code r) {
      Op* op;
      {
	shared_lock l(rwlock);

	auto p = inflight_ops.find(tid);
    ...
    template<typename D>
    bool Objecter<D>::target_should_be_paused(Op& t) {
      bool pauserd = osdmap->test_flag(CEPH_OSDMAP_PAUSERD);
      bool pausewr = osdmap->test_flag(CEPH_OSDMAP_PAUSEWR) ||
	osdmap_full_flag();
     ...
     */
    template<typename D>
    bool Objecter<D>::osdmap_full_flag() const {
      return osdmap->test_flag(CEPH_OSDMAP_FULL);
    }

    template<typename D>
    int Objecter<D>::_calc_targets(Op& t, typename Op::unique_lock& ol) {
	...
	return TARGET_VOLUME_DNE;
      ...
      // operations. But it never hurts to fail spectacularly in case
      // I goof up.
      uint32_t i = 0;
      t.volume->place(
    ...
    template<typename D>
    void Objecter<D>::_cancel_op(Op& op) {
    ...
    template<typename D>
    void Objecter<D>::_finish_op(Op& op, typename Op::unique_lock& ol) {
    ...
    template<typename D>
    MOSDOp *Objecter<D>::_prepare_osd_subop(SubOp& subop) {
    ...
    template<typename D>
    bool Objecter<D>::possibly_complete_op(Op& op,
					   typename Op::unique_lock& ol,
					   bool do_or_die) {
    ...
    template<typename D>
    void Objecter<D>::get_fs_stats(statfs_callback&& onfinish) {
    ...
    template<typename D>
    void Objecter<D>::handle_fs_stats_reply(MStatfsReply *m) {
      unique_lock wl(rwlock);

      ldout(cct, 10) << "handle_fs_stats_reply " << *m << dendl;
      ceph_tid_t tid = m->get_tid();

      auto stiter = statfs_ops.find(tid);
    ...
    template<typename D>
    void Objecter<D>::statfs_op_cancel(ceph_tid_t tid, std::error_code r) {
      unique_lock wl(rwlock);
    ...
    template<typename D>
    void Objecter<D>::_finish_statfs_op(StatfsOp& op) {
      statfs_ops.erase(op);
    ...
    template<typename D>
    Volume Objecter<D>::vol_by_uuid(const boost::uuids::uuid& id) {
      shared_lock rl(rwlock);
      return osdmap->lookup_volume(id);
    }

    template<typename D>
    AVolRef Objecter<D>::attach_by_uuid(const boost::uuids::uuid& id) {
      shared_lock rl(rwlock);
      return osdmap->lookup_volume(id).attach(cct, *osdmap);
    }

    template<typename D>
    Volume Objecter<D>::vol_by_name(const string& name) {
      shared_lock rl(rwlock);
      return osdmap->lookup_volume(name);
    }

    template<typename D>
    AVolRef Objecter<D>::attach_by_name(const string& name) {
      shared_lock rl(rwlock);
