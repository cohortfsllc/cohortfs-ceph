...
bool PGOSD::op_has_sufficient_caps(PG *pg, MOSDOp *op)
{
...
bool PGOSD::OpWQ::_enqueue(PG *pg)
{
  pg->get();
...
void PGOSDService::queue_for_op(PG *pg)
{
  op_wq.queue(pg);
}
...
void PGOSD::handle_op_sub(OpRequestRef op)
{
  ...
  if ((m->get_flags() & CEPH_OSD_FLAG_PGOP) == 0 &&
      get_pgosdmap()->have_pg_pool(pool)) {
    pgid = get_pgosdmap()->raw_pg_to_pg(pgid);
  }
  ...
  if (!pg) {
    ...
    if (get_pgosdmap()->get_pg_acting_role(pgid, whoami) >= 0) {
    ...
    pgid = m->get_pg();
    if ((m->get_flags() & CEPH_OSD_FLAG_PGOP) == 0 &&
	get_pgosdmap(send_map)->have_pg_pool(pgid.pool())) {
      pgid = get_pgosdmap(send_map)->raw_pg_to_pg(pgid);
    }
    
    if (get_pgosdmap(send_map)->get_pg_acting_role(m->get_pg(), whoami)
	>= 0) {
      dout(7) << "dropping request; client will resend when they "
	"get new map" << dendl;
    } else {
