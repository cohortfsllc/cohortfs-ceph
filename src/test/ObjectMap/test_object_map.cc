...
public:
  ...
  void set_key(const string &objname, const string &key, const string &value) {
    set_key(oid_t(objname), key, value);
  }

  void set_xattr(const string &objname, const string &key, const string &value) {
    set_xattr(oid_t(objname),
	      key, value);
  }

  void set_key(oid_t hoid,
  ...
  void set_header(const string &objname, const string &value) {
    set_header(oid_t(objname), value);
  }

  void set_header(oid_t hoid,
		  const string &value) {
    bufferlist header;
  ...
  int get_header(const string &objname, string *value) {
    return get_header(oid_t(objname), value);
  }

  int get_header(oid_t hoid,
		 string *value) {
    bufferlist header;
  ...
  int get_xattr(const string &objname, const string &key, string *value) {
    return get_xattr(oid_t(objname), key, value);
  }

  int get_xattr(oid_t hoid,
		string key, string *value) {
    set<string> to_get;
    to_get.insert(key);
  ...
  int get_key(const string &objname, const string &key, string *value) {
    return get_key(oid_t(objname), key, value);
  }

  int get_key(oid_t hoid,
	      string key, string *value) {
    set<string> to_get;
    to_get.insert(key);
    map<string, bufferlist> got;
  ...
  void remove_key(const string &objname, const string &key) {
    remove_key(oid_t(objname), key);
  }

  void remove_key(oid_t hoid,
		  string key) {
  ...
  void remove_xattr(const string &objname, const string &key) {
    remove_xattr(oid_t(objname),
		 key);
  }

  void remove_xattr(oid_t hoid,
		    string key) {
    set<string> to_remove;
    to_remove.insert(key);
  ...
  void clear(oid_t hoid) {
    db->clear(hoid);
  }

  void clear_omap(const string &objname) {
    clear_omap(oid_t(objname));
  }

  void clear_omap(const oid_t &objname) {
...
TEST_F(ObjectMapTest, CreateOneObject) {
  oid_t hoid("foo", chunktype::entirety, 100);
...
TEST_F(ObjectMapTest, CloneOneObject) {
  oid_t hoid("foo", chunktype::entirety, 200);
  oid_t hoid2("foo2", chunktype::data, 201);
