...
private:
    cds::gc::HP hpGC;
public:
    CDS_Init() : hpGC(67) // magic number for SkipListSet HP impl
	{
	    cds::Initialize(0);
...
static CDS_Init cds_static_initializer;
...
static struct fio_option* init_options() {
	...
	options[0].name     = "cds_objectstore";
	options[0].lname    = "ceph cds objectstore type";
	options[0].type     = FIO_OPT_STR_STORE;
	options[0].help     = "Type of ObjectStore to create";
	options[0].off1     = offsetof(struct ceph_filestore_options,
				       objectstore);
	...
	options[1].off1     = offsetof(struct ceph_filestore_options,
				       filestore_debug);
	...
	options[2].off1     = offsetof(struct ceph_filestore_options,
				       filestore_journal);
...
static int _fio_setup_ceph_filestore_data(
    struct thread_data *td,
...
{
    struct ceph_filestore_data *ceph_filestore_data =
	(struct ceph_filestore_data *) td->io_ops->data;

    return ceph_filestore_data->aio_events[event];
}

static int fio_ceph_filestore_getevents(struct thread_data *td,
					unsigned int min,
...
static int fio_ceph_filestore_init(struct thread_data *td)
{
    ...
    struct ceph_filestore_data *ceph_filestore_data =
	(struct ceph_filestore_data *) td->io_ops->data;
    struct ceph_filestore_options *o =
	(struct ceph_filestore_options *) td->eo;
...
static void fio_ceph_filestore_io_u_free(struct thread_data *td,
					 struct io_u *io_u)
{
...
static int fio_ceph_filestore_io_u_init(struct thread_data *td,
					struct io_u *io_u)
{
    struct fio_ceph_filestore_iou *o;
