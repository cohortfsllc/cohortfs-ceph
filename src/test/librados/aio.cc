...
public:
  ...
  ~AioTestData()
  {
    if (m_init) {
      ...
      destroy_one_volume(m_volume_name, &m_cluster);
      sem_destroy(&m_sem);
    }
  }
  ...
  std::string init()
  {
    ...
    m_volume_name = get_temp_volume_name();
    std::string err = create_one_volume(m_volume_name, &m_cluster);
    if (!err.empty()) {
      sem_destroy(&m_sem);
      ostringstream oss;
      oss << "create_one_volume(" << m_volume_name << ") failed: error " << err;
      return oss.str();
    }
    ret = rados_ioctx_create(m_cluster, m_volume_name.c_str(), &m_ioctx);
    if (ret) {
      sem_destroy(&m_sem);
      destroy_one_volume(m_volume_name, &m_cluster);
      ostringstream oss;
...
public:
  ...
  ~AioTestDataPP()
  {
    if (m_init) {
      m_ioctx.close();
      destroy_one_volume_pp(m_volume_name, m_cluster);
      sem_destroy(&m_sem);
    }
  }
  ...
  std::string init()
  {
    ...
    m_volume_name = get_temp_volume_name();
    std::string err = create_one_volume_pp(m_volume_name, m_cluster);
    if (!err.empty()) {
      sem_destroy(&m_sem);
      ostringstream oss;
      oss << "create_one_volume(" << m_volume_name << ") failed: error " << err;
    ...
    ret = m_cluster.ioctx_create(m_volume_name.c_str(), m_ioctx);
    if (ret) {
      sem_destroy(&m_sem);
      destroy_one_volume_pp(m_volume_name, m_cluster);
      ostringstream oss;
...
TEST(LibRadosAio, OmapPP) {
  ...
  destroy_one_volume_pp(volume_name, cluster);
}
