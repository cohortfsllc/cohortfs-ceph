			     ...
			     OutTransaction *out)
{
  ...
  (void) out->t->push_oid(hobject_t(get<1>(obj)));
			      ...
			      OutTransaction *out)
{
  ...
  out->in_flight->push_back(trans_elt(obj, set_content(obj, contents)));
}
...
void FileStoreTracker::clone_range(const obj_desc_t &from,
  ...
  o_fix = out->t->push_oid(hobject_t(get<1>(from)));
  o_tix = out->t->push_oid(hobject_t(get<1>(to)));
  out->t->clone_range(c_fix, o_fix, o_tix, offset, len, offset);
  out->in_flight->push_back(trans_elt(to, set_content(to, to_contents)));
}
...
void FileStoreTracker::verify(const obj_desc_t &obj, bool on_start) {
  ...
  ObjectHandle oh = store->get_object(ch, hobject_t(get<1>(obj)));
